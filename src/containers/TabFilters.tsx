...
interface FilterProps {
  ...
  setSolved: React.Dispatch<React.SetStateAction<string[]>>;
}
...
//
const TabFilters: React.FC<FilterProps> = (data) => {
  const [isOpenMoreFilter, setisOpenMoreFilter] = useState(false);
  //
  const [rangePrices, setRangePrices] = useState([100, 500]);
  const [categoriesState, setCategoriesState] = useState<string[]>([]);
  const [difficultiesState, setDifficultiesState] = useState<string[]>([])
  const [solvesState, setSolvesState] = useState<string[]>([])
  ...
  const handleChangeDifficulties = (checked: boolean, name: string) => {
    checked
      ? setDifficultiesState([...difficultiesState, name])
      : setDifficultiesState(difficultiesState.filter((i) => i !== name));
  };

  const handleChangeSolves = (checked: boolean, name: string) => {
    checked
        ? setSolvesState([...solvesState, name])
        : setSolvesState(solvesState.filter((i) => i !== name));
  };
  //
                ...
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  ...
                  <div className="p-5 bg-neutral-50 dark:bg-neutral-900 dark:border-t dark:border-neutral-800 flex items-center justify-between">
                    ...
                    <ButtonPrimary
                      onClick={() => {
                        close()
                        data.setCategory(categoriesState)
                      }}
                      sizeClass="px-4 py-2 sm:px-5"
                    >
                      Apply
          ...
          <>
            ...
            >
              ...
              {!difficultiesState.length ? (
                ...
                <span onClick={() => setDifficultiesState([])}>{renderXClear()}</span>
              )}
            </Popover.Button>
            <Transition
              as={Fragment}
            ...
            >
              <Popover.Panel className="absolute z-40 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 lg:max-w-sm">
                <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                  <div className="relative flex flex-col px-5 py-6 space-y-5">
                    {DATA_difficulties.map((item) => (
                      <div key={item.name} className="">
                        <Checkbox
                          name={item.name}
                          label={item.name}
                          defaultChecked={difficultiesState.includes(item.name)}
                          onChange={(checked) =>
                            handleChangeDifficulties(checked, item.name)
                          }
                        />
                      </div>
                    ))}
                  ...
                  <div className="p-5 bg-slate-50 dark:bg-slate-900 dark:border-t dark:border-slate-800 flex items-center justify-between">
                    ...
                    <ButtonPrimary
                      onClick={()=>{
                        close()
                        data.setDifficulty(difficultiesState)
                      }}
                      sizeClass="px-4 py-2 sm:px-5"
                    >
                      Apply
              ...
              ${open ? "!border-primary-500 " : ""}
                ...
                >
                  ...
                  {!solvesState.length ? (
                      ...
                      <span onClick={() => setSolvesState([])}>{renderXClear()}</span>
                  )}
                </Popover.Button>
                <Transition
                    as={Fragment}
                ...
                >
                  <Popover.Panel className="absolute z-40 w-screen max-w-sm px-4 mt-3 left-0 sm:px-0 lg:max-w-sm">
                    <div className="overflow-hidden rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700">
                      <div className="relative flex flex-col px-5 py-6 space-y-5">
                        {DATA_solvings.map((item) => (
                            <div key={item.name} className="">
                              <Checkbox
                                  name={item.name}
                                  label={item.name}
                                  defaultChecked={solvesState.includes(item.name)}
                                  onChange={(checked) =>
                                      handleChangeSolves(checked, item.name)
                                  }
                              />
                            </div>
                        ))}
                ...
                <div className="inline-flex flex-col w-full text-left align-middle transition-all transform bg-white dark:bg-neutral-900 dark:border dark:border-neutral-700 dark:text-neutral-100 h-full">
                  ...
                  <div className="flex-grow overflow-y-auto">
                    <div className="px-6 sm:px-8 md:px-10 divide-y divide-neutral-200 dark:divide-neutral-800">
                      ...
                      {/* ---- */}
                      <div className="py-7">
                        <h3 className="text-xl font-medium">Difficulty</h3>
                        <div className="mt-6 relative ">
                          {renderMoreFilterItem(DATA_difficulties)}
                        </div>
                      </div>

                      {/* ---- */}
                      <div className="py-7">
                        <h3 className="text-xl font-medium">Solved</h3>
                        <div className="mt-6 relative ">
                          {renderMoreFilterItem(DATA_solvings)}
                        </div>
                      </div>
